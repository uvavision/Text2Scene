...
class EmbeddingModel(nn.Module):
    ...
    def forward(self, inputs):
        input_inds, input_lens, input_bg_imgs, input_fg_onehots, positive_patches, negative_patches = inputs
        inf_outs, env = self.teacher_forcing(input_inds, input_lens, input_bg_imgs, input_fg_onehots)
        positive_feats = self.shape_encoder(positive_patches)
        negative_feats = self.shape_encoder(negative_patches)
    ...
    def inference(self, input_inds, input_lens,
            start_step, explore_rate, explore_mode,
            ref_inds, ref_vecs):
        """
        Inputs:
    ...
    def scheduled_sample_step(self, env, prev_states, text_states, \
        ...
        if explore_rate < self.cfg.eps and (ref_vecs is not None):
            where_vecs = ref_vecs[:, 0].clone()
        else:
            where_vecs = expl_vecs.clone()
        # hack now, to test whether the embedding works or not
        where_vecs = self.decode_vectors(patch_vectors, where_inds[:, 0].unsqueeze(-1))
    ...
    def decode_vectors(self, input_patch_vectors, sample_coord_inds):
        patch_vectors = input_patch_vectors.squeeze(1)
        bsize, tsize, grid_dim = patch_vectors.size()
        aux_pos_inds = sample_coord_inds.expand(bsize, tsize).unsqueeze(-1)
        sample_patch_vectors = torch.gather(patch_vectors, -1, aux_pos_inds).squeeze(-1)
        return sample_patch_vectors
